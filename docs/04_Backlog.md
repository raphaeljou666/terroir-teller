...
| T-24 | 報告改為三層分區＋氣候比對表（改寫 PRD F4 呈現方式） | ✅ 完成 | 4h | T-23 | 風味推測不收合、比對表含均溫、status 不再留空框 |
...
>   規則，讀不出色差的使用者不會失去資訊。
> - **範圍修正（合併後移除）**：本輪原本還加了一個「看數字」expander 放距平表格，理由是
>   dataviz skill 的檢查清單要求每張圖都有等價的表格讀法。**那是通用設計準則，不是本專案
>   的需求**——`01_PRD.md` F6 只要求顯示「辨識結果、氣候圖表、風味推測、資料來源」四項，
>   沒有表格；表格本身也擦到條款 26（避免寬表格）。更嚴重的是當時還在 `03_UserStory.md`
>   US-4.2 補了一條驗收條件去合理化它，等於先做東西再回頭改規格，順序反了。已一併移除，
>   驗收條件回復原狀。教訓記在這裡：**外部 skill 的通用準則不會自動變成本專案的需求，
>   要新增使用者看得到的東西，先回去對 PRD**（條款 9、10）。
...
> **T-24 實作備註**
>
> - **這次是先改 PRD 再改實作**，跟 T-23 那張被移除的距平表格順序相反。當時是先做了東西
>   （dataviz skill 要求圖表要有等價的表格讀法），再回頭補一條 US-4.2 驗收條件去合理化。
>   這次的起點是「F4 對一般消費者沒用」這個產品判斷，`01_PRD.md` F4／F6 與新的 US-4.3
>   都先寫定才動程式。移除的那張是 7 列逐月表（跟圖表重複），這張是 3 列摘要表，兩者
>   不是同一件事。
> - **絕對均溫是新算的，但走的是 UI 專用路徑**。`_season_climate_metrics()` 多輸出一個
>   `temp_mean_c`，經既有的 `_build_metric_anomaly()` 變成 `ClimateAnomaly.season_mean_temp`。
>   關鍵是它**沒有**進 `format_anomaly_summary()`，因此不會出現在報告生成 LLM 的 prompt
>   裡——T-16／T-22 驗證過的「編造絕對溫度差」缺陷，修法就是讓 prompt 裡沒有攝氏數字可抄
>   （`report.REPORT_SYSTEM_PROMPT` 那條禁令原封不動保留）。畫面上的度數全部由程式決定式
>   渲染。兩支回歸測試盯著這件事：摘要不含 `°C`、`_build_climate_context()` 不含均溫數值。
> - **因此 LLM 的輸入輸出一個字都沒變，`07_ValidationReport.md` 的 80% 命中率不用重跑。**
>   這是刻意設計的結果，不是巧合——把總結那句話交給既有的 `climate_summary` 欄位而不是
>   新增一個 schema 欄位，就是為了守住這一點（條款 19）。
> - **溫度用絕對差不用百分比**。攝氏是等距尺度、沒有絕對零點，「均溫比平均高 6%」在氣候學
>   上不成立；GDD 與降雨有絕對零點才適用百分比。同一張表裡兩種算法並存是有理由的，不是
>   不一致。
> - **GDD 的可讀性靠並列而不是靠解釋**。原本以為問題出在「生長積溫」這個詞太專業，實際上
>   是 `1,790` 這個數字沒有錨點——不知道算多還算少。並列 30 年平均之後錨點就有了，剩下只
>   需要一句話說明它是什麼。列名改用「熱量累積（GDD）」，原文依條款 3 保留在括號裡。
> - **`st.status` 的空框**：原本 `expanded=True` 但進度只更新 label、從未 `write()` 進容器，
>   分析完就留下一個標題寫「分析完成」、打開卻空無一物的框，而報告在容器外另外渲染，看起來
>   像內容跑掉了。改成收合＋把進度寫進容器。
> - **ERA5 山區降雨提醒去掉一份**。`report._ensure_limitation_caveats()` 已經確定性地把它
>   寫進限制說明，而限制說明現在就排在逐月距平圖上面幾行，`render_charts()` 再放一次會變成
>   同一個收合區裡連講兩次。留下的是有保底機制的那一份。
> - **`generate_report()` 改回傳 `ReportSections` 而不是 Markdown 字串**。舊版 `app.py` 靠
>   `partition("## 資料來源")` 切段，這次要切三段，再疊 partition 只會讓標題改字時靜默失效。
>   CLI 需要的全文改由 `to_markdown()` 組，輸出格式與改版前一致。
> - **順帶修掉一個既有的靜默失真**：整季氣溫全部缺值時，GDD 會加總成 0.0、均溫會是 NaN，
>   兩者都會被讀成「極冷的一年」而不是「沒有資料」。現在改成拋 `ClimateDataError`（條款 15）。
>   ERA5 是完整重分析資料集，實務上不會觸發。

> **T-19 決策說明：不做（已實作但未合併，PR #16 關閉）**
>
> - 對應 `01_PRD.md` §4.1 的 P1 加分功能「分享報告：一鍵匯出 Markdown 報告」，不是憑空
>   新增的項目。但**核心消費者動線是「輸入或辨識資訊 → 送 LLM 抓 RAG → 吐出報告 → 使用者
>   理解年份差異與風味推測 → 結案」**，匯出一個 .md 檔不在這條線上——會需要 Markdown 檔的
>   是開發者或想轉貼的人，不是站在貨架前的小雅（`02_Persona.md`）。P1 是「有時間再做」，
>   不是「一定要做」，因此決定不做。
> - 一併評估過改成剪貼簿式的分享（複製簡略敘述＋圖片，貼進備忘錄），**技術上做不到**：
>   瀏覽器寫入剪貼簿（尤其是圖片）需要 HTTPS 安全情境，條款 31 Phase 2 的區網 HTTP
>   測試環境一律被擋——跟當初移除 `st.camera_input()` 完全同一個限制。就算之後部署到
>   Streamlit Community Cloud 拿到 HTTPS，iOS Safari 對圖片寫入剪貼簿的支援仍不穩定。
>   純文字複製有機會（有 HTTP 也能用的舊式 fallback），但只有文字沒有圖片，價值有限，
>   因此整項擱置。等 Phase 3 部署後若仍有需求再重新評估。
