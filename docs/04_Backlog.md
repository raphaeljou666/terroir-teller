...
| ID | 任務 | 狀態 | 預估 | 依賴 | 驗收條件 |
|---|---|---|---|---|---|
| T-16 | 知名年份驗證測試（3–5 案例） | ✅ 完成 | 3h | T-13 | 產出驗證報告 Markdown，見 `docs/07_ValidationReport.md`。修正後命中率 80%／90%，通過 PRD 門檻 |
| T-17 | 手動輸入 fallback 介面 | ✅ 完成 | 1h | T-14 | Vision 失敗時可切換手動 |
| T-18 | 錯誤處理與 loading 狀態優化 | ✅ 完成 | 2h | T-14 | 各種例外都有友善訊息 |
| T-19 | 一鍵匯出報告為 Markdown | ✅ 完成 | 1h | T-14 | 下載按鈕產生 .md 檔 |
...
| T-23 | 氣候距平圖表重新設計（改為逐月距平圖） | ✅ 完成 | 2h | T-15 | 雙圖合併為單一距平圖，縮放問題消失 |
...
> **T-23 實作備註**
>
> - **回饋有兩件事，一個改動同時解掉**。使用者說「兩張圖有點多餘，感覺取固定年區間呈現
>   差異比較就好」，以及「放大後沒有明顯的方式縮回原本的檢視範圍」。改成畫差值之後，
>   縮放本身也不需要了——7 根柱子的圖沒有放大的價值，兩軸直接設 `fixedrange=True` 關掉
>   縮放與平移。**沒有縮放就沒有要重置的狀態**，比補一顆「重置檢視」按鈕乾淨，也少一個
>   要在手機上點的小目標。
> - **溫度與降雨拆成上下兩排子圖，不是共用一張圖的兩個 y 軸**。°C 與 mm 尺度不同，疊在
>   雙 y 軸上會讓兩條線的交叉點變成純粹的視覺巧合（dataviz skill 把雙 y 軸列為最常見的
>   圖表錯誤）。用 `make_subplots(rows=2, shared_xaxes=True)` 共用月份軸，各自保留 y 軸。
> - **配色的角色變了，色碼沒變**。T-18 定的橘／藍原本是「該年 vs 30 年平均」的類別識別；
>   改畫差值之後那兩條序列不再並列，色碼轉為表達正負極性（橘＝高於平均、藍＝低於平均），
>   中點用中性灰。這是 dataviz skill 定義的 diverging 用法（兩個對立色相＋中性中點）。
>   已用 `validate_palette.js` 對 `.streamlit/config.toml` 的**實際底色**重新驗證
>   （亮 `#FFFDF9`／暗 `#1C1210`），兩種模式都 ALL CHECKS PASS，CVD 分離度
>   ΔE 24.7（protan）／26.8，遠高於 8 的門檻。
> - **顏色不是唯一的編碼**。柱子的正負方向本身就重複表達了同一件事，加上圖說明講明配色
>   規則，讀不出色差的使用者不會失去資訊。另外補一個「看數字」expander 放距平表格，
>   避免 tooltip 成為讀到數值的唯一入口（dataviz anti-pattern：a tooltip as the only way
>   to read a value / no table view）。表格只有三欄，手機不會爆版（條款 26）。
> - 兩個面板共用同一條配色規則：同一個顏色在兩張圖裡永遠代表同一件事。若讓「降雨偏多」
>   用藍色（水的直覺聯想），就會出現橘色在上排代表「高於平均」、在下排代表「低於平均」
>   的矛盾，這比違反水是藍色的聯想更容易誤讀。
> - 測試涵蓋：畫的是差值而非原始序列、顏色由正負決定、上下兩排不是雙 y 軸、兩軸都關閉
>   縮放、亮暗模式配色、表格與圖表數值一致。

> **T-19 實作備註**
>
> - 匯出的檔案會離開 app 被單獨閱讀或轉傳，抬頭因此要能自我說明：產區、年份、產生日期，
>   以及最重要的一句「這是從氣候資料推論出來的風味傾向，不是實際品飲筆記」。少了這句，
>   一份脫離脈絡的 Markdown 很容易被當成品酒評鑑看待，那正是 PRD 定位（可解釋的氣候脈絡
>   解讀層，不是評分）最不希望發生的誤讀。
> - 日期用 `YYYY-MM-DD` 並標註「當地時間」（條款 28、29）。
> - 檔名用產區的正式英文名稱、空白換連字號（`TerroirTeller_Napa-Valley_2018.md`），
>   不用中文檔名，避免跨作業系統的編碼問題。產區或年份缺值時退回 `unknown`，不拋例外。
> - `st.download_button` 點擊會觸發 rerun。分析邏輯本來就只寫在「開始分析」按鈕的分支裡、
>   結果存 `session_state`，所以不會重跑付費 API——但這是條款 19 的核心不變量，補了一條
>   回歸測試明確鎖住（跟既有的「編輯欄位不重跑」測試同一個模式）。

> **T-16 重新驗證結果（T-21／T-22 合併後）**
>
> - 用與第一次完全相同的判準與公開評論來源重跑 5 案例：**一致 4、部分一致 1、不一致 0**，
>   嚴格命中率 4/5 = **80%**、寬鬆 4.5/5 = **90%**，兩種算法都通過 PRD 的 ≥70% 門檻。
>   2013 Bordeaux 與 2011 Napa 兩案方向翻正；2019 Bordeaux 維持部分一致——陳年潛力的反向
>   結論與稀釋主軸都修好了，但系統仍推測「酸度偏低」，而公開評論把酸度與新鮮感視為該年的
>   定義性特徵，因此不因主軸修好就升級判定。即使把 2019 算成不一致，嚴格命中率仍是 80%，
>   結論不依賴那一案怎麼判。
> - `docs/07_ValidationReport.md` 兩份紀錄都保留：第一次驗證的原文（含當時對根因那個後來被
>   證明是錯的判斷）一字未改，修正後的結果另立章節。方法論檢驗的價值在於看得見判斷如何被
>   修正，不是只留最後好看的版本。
> - **規範遵從度機械檢查**：編造絕對溫度差 0 次（已根除）、簡體字或日文假名 0 次、5 份報告
>   的風味推測都有引用標記。但**陳年潛力仍有 1 次漏網**（2003 Bordeaux），T-22 把遵從度從
>   2/2 違規降到 1/5，沒有到 100%。這是 GPT-4o-mini 的指令遵從度限制，跟 T-13 記錄過的引用
>   標記問題同一類；要確定性得比照 `report._ensure_limitation_caveats()` 在程式端加後處理
>   過濾，列為下一輪可評估項目。
> - **成本**：整條修正鏈（T-21 + T-22 + 本次重新驗證）的完整 agent 呼叫約等於 8 個案例，
>   而不是「每改一次就重跑 5 案例」的 15 次以上。作法是先用離線測試與確定性檢索比對證明
>   機制正確（免費到不到一分錢），只在必要時才跑完整流程（條款 19）。
...
- **產區→慣用品種推論**（影響 T-13）：酒標辨識（T-10）的 `grape` 欄位為 `null`、但產區有效
  ...
  清單內的 20 個產區時才有意義。
- **陳年潛力說法的程式端後處理過濾**（T-16 重新驗證發現）：拒答清單第 5 項已在 T-22 從
  「具體年份預測」收緊成「任何形式的陳年潛力判斷」，遵從度從 2/2 違規降到 1/5，但沒有到
  100%——重新驗證時 2003 Bordeaux 仍寫出「也可能因此影響陳年潛力」。這是 GPT-4o-mini 的
  指令遵從度限制，跟 T-13 記錄過的引用標記問題同一類，光靠 prompt 補強沒辦法保證。若要
  確定性，可比照 `report._ensure_limitation_caveats()` 的 belt-and-suspenders 模式，在
  `report.py` 加一層後處理：偵測風味推測段落裡的陳年相關措辭（陳年潛力／適合久放／有利於
  陳年等）並移除或改寫。**設計上要小心兩件事**：(1) 不能誤殺合法的風味描述（「酸度偏高」
  本身沒問題，不該因為出現在陳年語句附近就被砍掉）；(2) 直接刪句可能讓段落讀起來斷裂，
  要決定是整段重寫還是只砍該子句。需要一組真實案例當測試素材再動工。

- ~~**氣候距平圖表呈現方式重新設計**（T-18 手機實測回饋）~~ → **已於 T-23 完成**，見上方
  P1 表格與實作備註。
