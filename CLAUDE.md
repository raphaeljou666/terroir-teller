...
**實作原則：數值事實由程式渲染，不交給 LLM 複述。**

畫面上每一個具體的數字（溫度、降雨、距平百分比、年份區間）都要從氣候資料算出來後直接
渲染，不要先寫進 prompt 再期待模型原樣抄回來。模型只負責解讀方向與推論，不負責搬運數字。

這條是從實際踩過的坑收斂出來的，不是預防性的潔癖：T-16 驗證時模型會編造「均溫比 30 年
平均高 X°C」這種句子，數字完全是憑空生成的。修法不是加更多「請不要編造」的指令，而是
讓 prompt 裡**根本沒有攝氏數字可抄**——`report.REPORT_SYSTEM_PROMPT` 明文禁止寫出溫度
差，`climate.format_anomaly_summary()` 也刻意不含絕對溫度。

因此新增任何氣候欄位時要先問一句：**它會不會流進 LLM 的 prompt？** T-24 新增的絕對均溫
走的是 UI 專用路徑（`ClimateAnomaly.season_mean_temp` → 比對表），沒有進摘要，所以那條
禁令仍然成立。要放寬禁令、讓模型自己寫數字，就必須連帶重跑 `07_ValidationReport.md` 的
案例並更新規範遵從度統計，不能只改 prompt 就當沒事。

同樣的精神也適用於「算不出來」的情況：`pct_anomaly` 為 `None` 時顯示「無法計算」，
不要填 0；整季氣溫全部缺值時寧可拋錯，也不要讓 GDD 加總出來的 0.0 被讀成「極冷的一年」。
...
- **拍照優先用 `st.camera_input()`**，會自動叫出手機相機——已覆寫規範 #26，見 T-18 實作
  ...
  後可重新評估要不要加回來
- **避免寬表格**（手機會橫向捲動很難用），改用垂直堆疊卡片——已部分覆寫規範 #26，見
  T-24 實作
  備註：這條擋的是「欄位多到要橫向捲」的表格，不是所有表格。T-24 的氣候比對表是
  3 列 × 4 欄、每格不超過 7 個字元（`19.4°C`、`1,580`、`+12.1%`），手機直向就放得下，
  而且它要傳達的正是「採收年 vs 30 年平均」的並列關係——拆成垂直堆疊卡片反而會把要比較
  的兩個數字拆到不同卡片上，讀者得自己在腦中對齊，比橫向捲動更難用。判準是**要不要橫向
  捲**，不是**是不是表格**。真正被這條擋掉的例子是 T-23 那張 7 列逐月距平表，已移除。
...
- [ ] 有新增氣候欄位或改動 prompt 嗎？它會不會流進 LLM 的 prompt、是否需要重跑
      `07_ValidationReport.md` 的案例？（條款 15 實作原則）
- [ ] 要新增使用者看得到的東西嗎？**先回去對 PRD**，不要做完再補驗收條件去合理化
      （T-23 的教訓，見 `04_Backlog.md`）
- [ ] 完成後要更新哪些文件？
